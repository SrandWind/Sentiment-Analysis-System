...
function transformComparisonData(metrics: ComparisonMetrics[]): {
  ...
  const performanceData: PerformanceMetric[] = [
    ...
    {
      metric: 'MBTI Acc',
      base: getModelMetric(matchModel('base'), m => (m.mbti_accuracy || 0) * 100),
      lora: getModelMetric(matchModel('lora'), m => (m.mbti_accuracy || 0) * 100),
      gguf: getModelMetric(matchModel('gguf'), m => (m.mbti_accuracy || 0) * 100),
      unit: '%',
      isMaeRow: false,
      lowerIsBetter: false,
      maxVal: 20,
    ...
    {
      metric: 'JSON Parse',
      base: getModelMetric(matchModel('base'), m => (m.json_parse_rate || 0) * 100),
      lora: getModelMetric(matchModel('lora'), m => (m.json_parse_rate || 0) * 100),
      gguf: getModelMetric(matchModel('gguf'), m => (m.json_parse_rate || 0) * 100),
      unit: '%',
      isMaeRow: false,
      lowerIsBetter: false,
      maxVal: 100,
